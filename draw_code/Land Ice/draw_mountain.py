...
ds_qs = xr.apply_ufunc(quantile_1D,
                       mg_esls.chunk(dict(icemod=-1)).astype(float),  # [100:,:].
                       mg_wts.chunk(dict(icemod=-1)).astype(float),  # [100:]
                       ds_q,
                       vectorize=True,
                       dask="parallelized",
...
font_style={'family':'Times New Roman',
            'size':10}
ax.set_ylabel('Ice volume (10^6 Gt)',font=font_style)

ax.fill_between(ages, value_90down, value_90up, color='#d5dadd', alpha=0.4,  label="90% credible interval")
ax.fill_between(ages, value_66down, value_66up, color='#a4b9ca', alpha=0.4,  label="66% credible interval")
...
ax.plot(th_time,np.full_like(th_time,th95_Ho),label='Holocene Baseline 95%',color='#ff0001')
...
ax.set_xlim(left=11.7)

plt.gca().yaxis.set_major_locator(ticker.MaxNLocator(4))
plt.show()
# out_path = r"./Extended Data Fig 1d.svg"
