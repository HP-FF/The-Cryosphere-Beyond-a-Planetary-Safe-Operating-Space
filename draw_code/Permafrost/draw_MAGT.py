...
if __name__ == "__main__":
    excel_folder = r"../../draw_data/permafrost/MAGT"
    ...
    for expath in ex_folder:
        ...
        if os.path.basename(expath).split('_')[1].split('.')[0] == 'CESM2':
            ...
            yh_95 = np.percentile(y_h, 95)
            yn_95 = np.percentile(y_n[0:50],95)
            print("current file：{}，95%:{}".format(os.path.basename(expath), yh_95))
            print("current file：{}，95_now%:{}".format(os.path.basename(expath), yn_95))
            ...
            if color_index == 0:
                ...
                plt.plot(x[x <= bias-gap], [yh_95] * len(x[x <= bias-gap]), c='#2626ff', linestyle='--', alpha=0.8, linewidth=2,label='Mid-Holocene baseline based on CESM2')
                plt.plot(x[x >= bias], [yh_95] * len(x[x >= bias]), c='#2626ff', linestyle='--', alpha=0.8,
                         linewidth=2)

        else:
            if os.path.basename(expath).split('_')[1].split('.')[0] == 'AME':
                yn_95 = np.percentile(y_n[0:50],95)
                print("current file：{}，95_H%:{}".format(os.path.basename(expath), yn_95))
        ...
        plt.ylabel('Permafrost MAGT (℃)')  # y_label

    plt.show()
    # out_path = './Extended Data Fig 3a.svg'
