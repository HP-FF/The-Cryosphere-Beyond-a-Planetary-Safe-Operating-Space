...
if __name__ == "__main__":
    excel_folder = "../../draw_data/Snow Cover"
    ...
    for expath in ex_folder:
        ...
        if os.path.basename(expath).split('_')[1].split('.')[0] == 'CESM2':
            ...
            if os.path.basename(expath).split('_')[1].split('.')[0] == 'AME':
                ...
                plt.plot(x, [yn_05] * len(x[x >= bias]), c='red', linestyle='--', alpha=0.8, linewidth=2,label='Pre-industrial baseline based on multimodel mean')

                plt.plot(x_n, y_n, c='#0b0b0b', linestyle='-', alpha=0.8, linewidth=2, label='Multimodel mean, 1850-2014')

            elif os.path.basename(expath).split('_')[1].split('.')[0] == 'Era5-Land':
                plt.plot(x_n, y_n, c='#fea443', linestyle='-', alpha=0.8, linewidth=2,label='ERA5-Land data')

        plt.legend(loc='lower left')
        plt.ylabel('Snow Area ($10^6\ km^2$)')
